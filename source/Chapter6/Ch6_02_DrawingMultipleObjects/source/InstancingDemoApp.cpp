...
InstancingDemoApp::InstancingDemoApp()
{
	...
	//m_Cube = new RectangleFactory(this);

	m_CubeFactory = RectangleFactory::SingleTon(this);

	m_Scene = new Scene3D();

	m_Cube = new RectangleModel(this, m_Scene, NULL, "Rectangle 1", SHAPE_RECTANGLE);
...
void InstancingDemoApp::Setup()
{
	...
	m_Scene->Update(); // Force it once so that the flat list is generated

	m_CubeFactory->PrepareMultipleDataObjects(m_Scene);
