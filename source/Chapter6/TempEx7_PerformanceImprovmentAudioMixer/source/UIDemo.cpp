...
void UIDemo::Grid(Scene* p_Scene, int p_Width, int p_Height)
{
    ...
    for (int i = 0; i < parentCol; i++)
    {
        ...
        {
            Node* m_Parent = new Rectangl(p_Scene, NULL, BoundingRegion((i * parentColWidth), (j * parentColHeight), parentColWidth - 2, parentColHeight), "Node 1", SHAPE::SHAPE_RECTANGLE_INSTANCED);
            ...
            for (int k = 0; k < Col; k++)
            {
                for (int l = 0; l < Row; l++)
                {
                    Rectangl* rect = new Rectangl(p_Scene, m_Parent, BoundingRegion((k * colWidth), (l * colHeight), colWidth, colHeight), "Node 1", SHAPE::SHAPE_RECTANGLE_INSTANCED);
    ...
    : Node(p_Scene, p_Parent, p_BoundedRegion, p_Name, p_ShapeType)
{
    ...
    Rectangl* background = new Rectangl(p_Scene, this, bgDim, "", SHAPE::SHAPE_RECTANGLE_INSTANCED);
    ...
    bar = new Rectangl(p_Scene, background, barDim, "Bar", SHAPE::SHAPE_RECTANGLE_INSTANCED);
    ...
    progressIndicator = new Rectangl(p_Scene, bar, piDim, "ProgressIndicator", SHAPE::SHAPE_RECTANGLE_INSTANCED);
...
AudioMixerItem::AudioMixerItem(Scene* p_Scene, Node* p_Parent, const BoundingRegion& p_BoundedRegion, const QString& p_Name, SHAPE p_ShapeType)
    : Node(p_Scene, p_Parent, p_BoundedRegion, p_Name, p_ShapeType)
{
    Node* background = new Rectangl(m_Scene, this, BoundingRegion(0, 0, p_BoundedRegion.m_Dimension.x, p_BoundedRegion.m_Dimension.y), "Audio Mixer Background", SHAPE::SHAPE_RECTANGLE_INSTANCED);
    ...
    Node* activeTrackIndicator = new Rectangl(m_Scene, background, BoundingRegion(activeTrackIndicatorTopMarginLeftMargin, activeTrackIndicatorTopMargin, p_BoundedRegion.m_Dimension.x - (5 * activeTrackIndicatorTopMarginLeftMargin), activeIndicatorWidth), "Active Track Indicator", SHAPE::SHAPE_RECTANGLE_INSTANCED);
    ...
    {
        Node* channelBackground = new Rectangl(m_Scene, background, BoundingRegion((i * channelWidth) + channelLeftMargin, channelTopMargin, ((i == (formatType - 1)) ? 2 : 0) + channelWidth, p_BoundedRegion.m_Dimension.y - channelTopMargin - 5.0), "Channel", SHAPE::SHAPE_RECTANGLE_INSTANCED);
        ...
        Node* channel = new Rectangl(m_Scene, channelBackground, BoundingRegion(2, 2, channelWidth - 2, p_BoundedRegion.m_Dimension.y - channelTopMargin - 5.0 - 4), "Channel", SHAPE::SHAPE_RECTANGLE_INSTANCED);
        ...
        {
            Node* levelIndicator = new Rectangl(m_Scene, channel, BoundingRegion(2, j * 4, channelWidth - 4.0, 2.0), "Channel", SHAPE::SHAPE_RECTANGLE_INSTANCED);
